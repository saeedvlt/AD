...
def adjustment_status(section: str, line_item: str) -> str:
    """Classify adjustment rows for Material."""
    if section != "Material":
        return "Standard"
...
def convert(uploaded_file: Any) -> pd.DataFrame:
    ...
    for location in LOCATIONS:
        ...
        material_seen={"Fabrications":0,"Customer Material":0}
                ...
                records.append(
                    {
                        ...
                        "Line Item": label,
                        "Adjustment Status": (
                            "Adjustment" if label=="Adjust to Actual" and current_section=="Material"
                            else (
                                "Original" if current_section=="Material" and label in material_seen and (material_seen.__setitem__(label, material_seen[label]+1) or material_seen[label])==1
                                else (
                                    "Net" if current_section=="Material" and label in material_seen and material_seen[label]>=2
                                    else "Standard"
                                )
                            )
                        ),
